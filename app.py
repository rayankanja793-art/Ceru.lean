...
# ==========================================
# 1. CORE DATA REGISTRIES & LEAGUE STATE
# ==========================================
USERS_DB = {
    'admin@ligibigi.com': {'phone': '00000000', 'password': 'adminpassword', 'balance': 9999999.0, 'is_admin': True}
}
...
# Initialize persistent standing matrix for league tracker
LEAGUE_STANDINGS = {
    ...
    while True:
        ...
        with lock:
            ...
            if ENGINE_STATE['phase'] == 'BETTING' and ENGINE_STATE['time_remaining'] <= 0:
                ...
                settle_all_round_bets()
                update_league_standings() # Process league progression numbers
...
def update_league_standings():
    """Accumulates league metrics across all 20 running team objects"""
    global LEAGUE_STANDINGS, ENGINE_STATE
    for match in ENGINE_STATE['active_matches']:
        h, a = match['home'], match['away']
        hg, ag = match['final_h_g'], match['final_a_g']
        
        LEAGUE_STANDINGS[h]["P"] += 1
        LEAGUE_STANDINGS[a]["P"] += 1
        LEAGUE_STANDINGS[h]["GF"] += hg
        LEAGUE_STANDINGS[h]["GA"] += ag
        LEAGUE_STANDINGS[a]["GF"] += ag
        LEAGUE_STANDINGS[a]["GA"] += hg
        
        if hg > ag:
            LEAGUE_STANDINGS[h]["W"] += 1
            LEAGUE_STANDINGS[h]["PTS"] += 3
            LEAGUE_STANDINGS[a]["L"] += 1
        elif hg < ag:
            LEAGUE_STANDINGS[a]["W"] += 1
            LEAGUE_STANDINGS[a]["PTS"] += 3
            LEAGUE_STANDINGS[h]["L"] += 1
        else:
            LEAGUE_STANDINGS[h]["D"] += 1
            LEAGUE_STANDINGS[h]["PTS"] += 1
            LEAGUE_STANDINGS[a]["D"] += 1
            LEAGUE_STANDINGS[a]["PTS"] += 1
            
        LEAGUE_STANDINGS[h]["GD"] = LEAGUE_STANDINGS[h]["GF"] - LEAGUE_STANDINGS[h]["GA"]
        LEAGUE_STANDINGS[a]["GD"] = LEAGUE_STANDINGS[a]["GF"] - LEAGUE_STANDINGS[a]["GA"]

def get_sorted_standings():
    """Returns a list of teams sorted by Points, Goal Difference, then Goals For"""
    standings_list = []
    for team, stats in LEAGUE_STANDINGS.items():
        row = {"team": team}
        row.update(stats)
        standings_list.append(row)
    # Python sorts stably; sort reverse by key hierarchy priority
...
<style>
    ...
    .grid-matrix { display: grid; grid-template-columns: 1.8fr 1.2fr; gap: 20px; }
    .card { background: #16222f; border-radius: 8px; border: 1px solid #223245; padding: 15px; margin-bottom: 15px; }
    .timer-banner { text-align: center; font-size: 16px; font-weight: bold; padding: 12px; border-radius: 6px; margin-bottom: 15px; letter-spacing: 0.5px; transition: background 0.3s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
    .timer-banner.betting { background: #006633; color: white; border: 1px solid #00ff66; }
    .timer-banner.playing { background: #b20000; color: white; border: 1px solid #ff3333; }
    .timer-banner.paused { background: #445566; color: white; border: 1px solid #8899aa; }
    
    .table-match { width: 100%; border-collapse: collapse; margin-top: 10px; }
    .table-match th { text-align: left; background: #0f1822; padding: 10px; color: #8fa1b4; font-size: 11px; text-transform: uppercase; }
    .table-match td { padding: 10px 8px; border-bottom: 1px solid #1f2d3d; font-size: 13px; }
    .odds-group { display: flex; gap: 5px; }
    .odds-btn { background: #213244; border: 1px solid #2d425a; color: #00ff66; padding: 6px; border-radius: 4px; text-align: center; cursor: pointer; font-weight: bold; flex: 1; min-width:55px; font-size:11px;}
    .odds-btn:hover { background: #2d425a; }
...
<div class="wrapper">
    <div id="live-timer-banner" class="timer-banner betting">Syncing system engine loop parameters...</div>
    ...
    <div class="grid-matrix">
        <div>
            <div class="card">
                <h3 style="margin-top:0; border-bottom: 1px solid #223245; padding-bottom:8px; color: #00ff66;">Active Round Match Matrix</h3>
                <table class="table-match">
                    <thead>
                        <tr>
                            <th>Fixture Matchup</th>
                            <th style="width:220px; text-align:center;">1X2 Selection Odds</th>
                        </tr>
                    </thead>
                    <tbody id="match-rows-container">
                        {% for match in state.active_matches %}
                        ...
                        <tr>
                            <td>
                                <strong id="home-name-{{ match_idx }}">{{ match.home }}</strong> 
                                <span id="score-space-{{ match_idx }}" style="color:#ffcc00; margin: 0 10px; font-weight:bold;">vs</span> 
                                <strong id="away-name-{{ match_idx }}">{{ match.away }}</strong>
                            </td>
                            <td>
                                <div class="odds-group">
            ...
            <div class="card">
                <h3 style="margin-top:0; border-bottom: 1px solid #223245; padding-bottom:8px; color:#ffcc00;">Official League Standings Matrix</h3>
                <table class="table-match" style="text-align: center;">
                    <thead>
                        <tr>
                            <th style="text-align: left;">Pos / Team</th>
                            <th>P</th>
                            <th>W</th>
                            <th>D</th>
                            <th>L</th>
                            ...
                            <th>GA</th>
                            <th>GD</th>
                            <th style="color:#00ff66;">Pts</th>
                        </tr>
                    </thead>
                    <tbody id="standings-rows-container">
                        </tbody>
                </table>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="card">
                <h4 style="margin-top:0; color:#8fa1b4;">🕒 Recent Match Results Feed</h4>
                <div id="history-feed-box" style="max-height:350px; overflow-y:auto;">
                    </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <table class="table-match">
            ...
            <tbody>
                ...
                <tr>
                    <td>#{{ b.round }}</td>
                    <td>Slot Record Block #{{ b.match_idx + 1 }}</td>
                    <td>Market Pick: (<strong>{{ b.prediction }}</strong>)</td>
                    <td>KSh {{ b.stake }}</td>
                    <td>{{ b.odds }}</td>
                    <td>
...
function syncEngineState() {
    fetch('/api/state')
        ...
        .then(data => {
            ...
            // Perform clean interface transition reload sequence upon round alterations
            if (currentRound !== 0 && currentRound !== data.round_number) {
                location.reload();
                return;
            }
            ...
            // Rebuild Standings Table Elements dynamically
            let standingsContainer = document.getElementById('standings-rows-container');
            standingsContainer.innerHTML = "";
            data.standings.forEach((row, idx) => {
                let tr = document.createElement('tr');
                tr.innerHTML = `
                    <td style="text-align: left;"><strong>${idx + 1}.</strong> ${row.team}</td>
                    <td>${row.P}</td>
                    <td>${row.W}</td>
                    <td>${row.D}</td>
                    <td>${row.L}</td>
                    ...
                    <td style="color:#00ff66; font-weight:bold;">${row.PTS}</td>
                `;
                standingsContainer.appendChild(tr);
            });

            // Rebuild Match Result Component panels cleanly
            let histBox = document.getElementById('history-feed-box');
            histBox.innerHTML = "";
            if (data.history.length === 0) {
                histBox.innerHTML = `<p style="color:#667788; font-size:12px; margin:0;">Waiting for initial round simulation closure...</p>`;
            }
            data.history.forEach(r => {
                let cardDiv = document.createElement('div');
                cardDiv.style.background = "#0f1822";
                cardDiv.style.padding = "8px 12px";
                cardDiv.style.borderRadius = "6px";
                cardDiv.style.marginBottom = "8px";
                cardDiv.style.borderLeft = "3px solid #ffcc00";
                
                let scoresHtml = r.results.map(res => `<div style="padding:3px 0; color:#ddd; font-size:12px;">• ${res}</div>`).join("");
                cardDiv.innerHTML = `
                    <div style="color:#ffcc00; font-weight:bold; font-size:12px; margin-bottom:4px;">ROUND #${r.round} FINAL OUTCOMES</div>
                    ${scoresHtml}
                `;
                histBox.appendChild(cardDiv);
            });
        });
}
...
def api_state():
    with lock:
        return jsonify({
            ...
            'history': COMPLETED_ROUNDS_HISTORY[:6],
            'standings': get_sorted_standings()  # Injects real-time dynamically calculated standings
        })
